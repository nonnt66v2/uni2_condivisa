...
#define DHT_PIN 3        // GPIO pin where DHT11 is connected
  ...
  while (true) {
    ...
    if (ret == 1) { // 1 means success
      ...
      while (true) {
        ...
        char message[64];
          snprintf(message, sizeof(message), "DHT sensor error - Message #%d\n", ++message_count);
        ret = esp_tls_conn_write(tls, message, strlen(message));
        } else {
          // Send sensor data to server
          char message[128];
          snprintf(message, sizeof(message), 
                   "Temp: %.2f°C, Humidity: %.2f%% - Message #%d\n", 
                   temperature, humidity, ++message_count);
          
          ret = esp_tls_conn_write(tls, message, strlen(message));
          Serial.printf("[%s] Sent sensor data: %s", TAG, message);
        }
        ...
        vTaskDelay(2000 / portTICK_PERIOD_MS); // Send sensor data every 2 seconds
      }
